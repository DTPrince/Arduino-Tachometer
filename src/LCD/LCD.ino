...
void loop() {
  ...
  freq = 1/period;  //Might have to check against period == 0 or error handle the black hole.
                    // This bit of code is also likely to disappear or be absorbed into the a functional implementation so I don't actually have to dedicate memory to it.
                    //  ^ depends on how often I need to call freq for readout.
                    // I could write a small algorithm to check the diff/period ratio and have it sample more frequently if the engine is accelerating.
                    // This might be too expensive but it would make the write cycles like a billion times less intensive. Usually.
